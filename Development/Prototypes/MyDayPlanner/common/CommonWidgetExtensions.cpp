...
namespace common {
    ...
    QLineEdit *lineEditWidthByCharCount(const char *objName, QWidget *parent, int charCount)
    {
        QLineEdit* newLineEdit = namedWidget<QLineEdit>(objName, parent);
    ...
    QFormLayout* formLayoutWithPolicy(const char* formName, QWidget* parent, QFormLayout::FieldGrowthPolicy policy)
    {
        QFormLayout* newFormLayout = namedWidget<QFormLayout>(formName, parent);
        newFormLayout->setFieldGrowthPolicy(policy);

        return newFormLayout;
    }

    QDateEdit *dateEditWithCalendar(const char *formName, QWidget *parent, QDate date)
    {
        const int TwoWeeks = 14;
        const int OneYear = 365;

        QDateEdit* newDateEdit = new QDateEdit(date, parent);
        ...
        newDateEdit->setMinimumDate(date.addDays(-TwoWeeks));
        
        newDateEdit->setMaximumDate(date.addDays(OneYear));

        return newDateEdit;
    }

    QPlainTextEdit *flexWidthPlainTextEdit(const char *objName, QWidget *parent, int boxWidthMin, int boxWidthMax, int rowCount)
    {
    ...
    QPlainTextEdit *flexPlainTextBasedOnCharCount(const char *objName, QWidget *parent, int minCharCount, int maxCharCount, int rowCount)
    {
        QPlainTextEdit* newPTextEdit = new QPlainTextEdit(parent);
    ...
    QTextEdit *flexWidthTextEdit(const char *objName, QWidget *parent, int boxWidthMin, int boxWidthMax, int rowCount)
    {
    ...
    QTextEdit *flexTextEditBasedOnCharCount(const char *objName, QWidget *parent, int minCharCount, int maxCharCount, int rowCount)
    {
        QTextEdit* newTextEdit = new QTextEdit(parent);
    ...
    int getMaxWidth(QFormLayout* formToSize)
    {
        int maxWidth = 0;
    ...
    int getMaxHeight(QFormLayout *formToSize)
    {
